...
{
    root->_color = Node::Color::RED;
    root->_lchild->_color = Node::Color::BLACK;
    root->_rchild->_color = Node::Color::BLACK;
}
...
void RedBlackBST<K, V>::flipColorCombine(Node *root)
{
    ...
    root->_lchild->_color = Node::Color::RED;
    root->_rchild->_color = Node::Color::RED;
}
...
int main(void)
{
    ...
    std::cout << tree.get("小明") << '\n';
    try
    {
        std::cout << tree.get("小猪");
    }
    catch (const std::exception &e)
    {
        std::cerr << e.what() << '\n';
